...
def save_articles_as_html_and_pdf():
    ...
    html_file.write((all_articles.encode("utf-8")).decode('utf-8'))
    html_file.close()
...
def scrape_category(category_url):
    try:
        soup = BeautifulSoup(requests.get(BASE_URL + category_url).text, "html.parser")
    ...
    for link in links:
        try:
            ...
            for code_tag in link_soup.find_all('pre', 'table'):
